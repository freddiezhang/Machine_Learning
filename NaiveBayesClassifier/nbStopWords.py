...
def train():
    ...
    totals = {x: cons.get(x,0) + libs.get(x,0) for x in set(cons).union(libs)}
    sortedlist = sorted(totals.items(), key = operator.itemgetter(1),reverse = True)
    i = 0
    N = int(sys.argv[3])
